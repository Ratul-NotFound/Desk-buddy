...
#define QUIRK_COUNT 14   // 14 unique autonomous personality quirks

class SoulEngine {
public:
    void init(DisplayEngine* disp, AudioEngine* audio, ServoEngine* servo);
    void update();   // call every 33ms from Core 1 soul task
    ...
    // BrainEngine sets this so mood-expression timer can show the right face
    void setCurrentMoodEmotion(RobotEmotion e) { _currentMoodEmotion = e; }
    ...
    void feedSnack() {
        _hunger          = min(100, _hunger + 30);
        _affection       = min(100, _affection + 8);
...
private:
    ...
    unsigned long _nextMoodExpress   = 0;
    ...
    // Mood expression: set by BrainEngine, shown periodically by SoulEngine
    RobotEmotion _currentMoodEmotion = EMOTION_IDLE;
    ...
    void   _doMoodExpression();
    void   _doAutoTalkCheck();
    void   _fallAsleep();
    void   _wakeUp();
    String _pickContextualTopic();
};
